...
public class TEncSbacV1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(buffer_fpga.size() != cpu.buffer.size()){
				System.out.println(String.format("Size Diff: CPU=%d, FPGA=%d", cpu.buffer.size(), buffer_fpga.size() ));
			}
			for(int j=0;j<buffer_fpga.size();j++){
				if(!buffer_fpga.get(j).equals(cpu.buffer.get(j))){
					System.out.println(String.format("[%d] Val Diff: CPU=%d, FPGA=%d", j, cpu.buffer.get(j)));
				}
			}
	...
	public ArrayList<Long> getOutput(int lastScanPos, int scanFirstPos){
		...
		if( (coded_sub_block_flag_compressed[0] & (1l << 8)) > 0l || defaultGroupFlagTrue){
			...
			//过滤remaining
			if(numGreater1Coded > 1 || ((coeff_abs_level_greater2_flag_compressed[0] & (1 << 8)) != 0) || numNonZero>8)//当coeff_abs_level_greater1_flag不为零的个数大于1 || coeff_abs_level_greater2_flag==1 || numNonZero>8，此值为true
