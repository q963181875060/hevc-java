...
public class EntropyEncoder {
	...
	public void bottomPartFinishAndWriteSingleOneAndAlignZero(){
		...
		int numOfBits =  13 + bitsLeft;
		numOfBits++;
		
		numBufferedBytes = numOfBits >> 3;
		int numOfRemainingBits = numOfBits - numBufferedBytes * 8;
		
		int bitsWithoutRemaining = bits >>> numOfRemainingBits;
		
		bufferedByte = bitsWithoutRemaining >>> (8 * (numBufferedBytes-1));
		...
		if(numBufferedBytes>0){
            ...
            while(numBufferedBytes>1){
            	numBufferedBytes -= 1;
            	int byte2write = bitsWithoutRemaining >>> (8 * (numBufferedBytes-1));
            	outputByte(byte2write);
            }
    	}

		bufferedByte = (bits << (8 - numOfRemainingBits)) & (0x0ff);
	}
		   ...
		   while((line = br.readLine()) != null){
			   ...
			   if(x[0]==1){
				  ...
				  /* if(x[2] == 151){
				   ...
				   enc0.finish();
			   }else if(x[0]==6){//m_pcTComBitIf->write( m_uiLow >> 8, 24 - m_bitsLeft ), write 1 bit 1
				   enc0.bottomPartFinishAndWriteSingleOneAndAlignZero();
			   }else if(x[0] == 7){//output the last partial byte if it is not 0
				   enc0.alignZero();
			   }else if(x[0]==0){
